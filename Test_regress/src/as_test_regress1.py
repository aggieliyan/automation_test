...
class Test(unittest.TestCase):
    ...
    def add_cate(self):
        ...
        try:
            cate_management.add_cate(self.cfg, self.driver, cate_name=cate_name)
        except Exception, e:
            print traceback.format_exc() 
        finally:
    ...
    def prepaid_cardgroup(self):#充值卡
        ...
        #建卡组
        try:
            card_management.add_prepaid_cardgroup(self.cfg, self.driver, self.org_name, group_name=title, group_price=price)
        except Exception, e:
            print traceback.format_exc() 
        finally:
...
#        print self.p_card_pwd
    ...
    def course_cardgroup(self):
        ...
        try:
            card_management.add_course_cardgroup(self.cfg, self.driver, self.org_name, group_name=title)
        except Exception, e:
            print traceback.format_exc() 
        finally:
    ...
    def cate_cardgroup(self):
        ...
        try:
            card_management.add_cate_cardgroup(self.cfg, self.driver, self.org_name, title)
        except Exception, e:
            print traceback.format_exc() 
        finally:
    ...
    def buy_listen_card(self):
        self.total += 1
        try:
            card_management.buy_listen_card(self.cfg, self.driver)
        except Exception, e:
            print traceback.format_exc() 
    ...
    def listen_cardgroup(self):
        ...
        try:
            card_management.add_listen_cardgroup(self.cfg, self.driver, self.org_name, group_name=title)
        except Exception, e:
            print traceback.format_exc() 
        finally:
...
#           print self.l_card_num
    ...
    def add_and_get_card(self, card_type=0):#添加卡并返回第一个卡号和密码
        ...
        try:
            card_management.add_card(self.cfg, self.driver, self.org_name, card_prifix)
            if card_type == 0:
                time.sleep(3)
    ...
    def add_exam_card(self):
        ...
        academy = "qqhru"
        try:
            self.examcard_num = card_management.add_exam_card(self.cfg, self.driver, count, academy)
        except Exception,e:
            print traceback.format_exc() 
    ...
    def use_exam_card(self):
        ...
        try:
            if self.examcard_num != 0:
                card_management.user_usexamcard(self.cfg, self.driver, self.examcard_num)
            else:
                print u"没有获取到考试卡号"
        except Exception, e:
            print traceback.format_exc() 
    ...
    def add_course_to_cate(self):
        ...
        actual_name = "0"
        try:
            course_name = cate_management.add_courese_to_cate(self.cfg, self.driver)
    ...
    def import_one_student(self):
        self.total += 1
        self.import_name = 'exam4000'
        self.org_name = "stu_gy"       
        #单个导入学员
        try:
            student_management.import_one_student(self.cfg, self.driver, \
                self.org_name, self.import_name)
        except Exception, e:
            print traceback.format_exc() 
        finally:
    ...
    def use_prepaidcard(self):
        ...
        try:
            if self.p_card_num != 0 and self.p_card_pwd != 0:
                confirm_num = card_management.use_prepaidorcate_card(self.cfg, self.driver, self.p_card_num, self.p_card_pwd)
            else:
    ...
    def use_coursecard(self):#充课卡
        ...
        try:
            if self.c_card_num != 0 and self.c_card_pwd != 0:
                confirm_num = card_management.use_prepaidorcate_card(self.cfg, self.driver, self.c_card_num, self.c_card_pwd)
            else:
    ...
    def use_catecard(self):#补课卡
        ...
        try:
            if self.ca_card_num != 0 and self.ca_card_pwd != 0:
                course_num = card_management.use_course_card(self.cfg, self.driver, self.ca_card_num, self.ca_card_pwd)
            else:
...
#        paper_name = self.paper_name
        paper_name = u'综合'
        try:
            exam_user_management.exam_user(self.cfg, self.driver, operation, blank_pager, question_answer, paper_name)
        except Exception, e:
            print traceback.format_exc() 
