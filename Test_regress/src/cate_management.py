...
def add_cate(cfg,driver, base_url,org_name, cate_name=u'计算机'):
    ...
    driver.find_element(cfg.get('org_manage','cate_name_by'),cfg.get('org_manage','cate_name')).send_keys(cate_name)
...
def delete_cate(cfg,driver, base_url, org_name):
    ...
    time.sleep(2)
    driver.find_element(cfg.get('org_manage','delete_cate_ok_by'),cfg.get('org_manage','delete_cate_ok')).click()
    time.sleep(2)
...
def add_courese_to_cate(cfg,driver, base_url, org_name, cate_num = 0):
    ...
    driver.find_element(cfg.get('org_manage','course_add_ok_by'),cfg.get('org_manage','course_add_ok')).click()
    time.sleep(1) 
