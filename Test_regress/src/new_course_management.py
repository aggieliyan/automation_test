    ...
    if ctype == 1:
        ...
        if isthree == 1:#发三分屏
            ...
            #本地上传
                ...
                driver.execute_script("$('.fileinput-button input').eq(0).attr('style',\
                    'height:20px;opacity:1;display:block;position:static;transform:translate(0px, 0px) scale(1)')")
                driver.implicitly_wait(1)
...
def forsale_couse(driver, base_url):
    filepath = r"W:\Testing\auto_test_file\headpic.jpg"
    driver.get("%smyOffice.do" %(base_url))
    driver.implicitly_wait(2)
